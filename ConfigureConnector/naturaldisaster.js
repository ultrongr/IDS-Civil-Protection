...
// Use the Naturaldisaster env file
dotenv.config({ path: 'naturaldisaster.env' });
...
const {
  // URLs should point to your running connectors
  PROVIDER_URL,
  PROVIDER_ALIAS,
  CONSUMER_URL,
  BROKER_API,
  BROKER_REV_PROXY,
  DATA_API_URL,
  DATA_DOC_URL, // optional: docs endpoint if you have one
} = process.env;
...
try {
  ...
  const offerResp = await axios.post(
    ...
    {
      title: 'Naturaldisaster Data Offer',
      description: 'Directory of Naturaldisaster entries according to the 112 service',
      keywords: ['NATURALDISASTER', 'directory', 'disaster', '112'],
      publisher: DATA_API_URL,
      language: 'EN',
      sovereign: DATA_API_URL,
      endpointDocumentation: DATA_DOC_URL || DATA_API_URL,
  ...
  const catResp = await axios.post(`${PROVIDER_URL}/api/catalogs`, {
    title: 'Naturaldisaster Data Catalog',
    description: 'Catalog for Naturaldisaster offers',
  });
  ...
  const cdResp = await axios.post(`${PROVIDER_URL}/api/contracts`, {
    title: 'Contract',
    description: 'Contract for sharing Naturaldisaster data',
    provider: PROVIDER_ALIAS, // ensure this matches your connector’s self-description base URL
  ...
  // 7. Create Artifact — formalized JSON value (no token/encryption)
  await new Promise((res) => setTimeout(res, waitTime));
  console.log('Creating artifact (formalized JSON)');

  const artifactConfig = {
    type: 'open-data-endpoint',
    version: 1,
    dataset: 'naturaldisaster-112',
    endpoint: {
      url: DATA_API_URL,
      method: 'GET',
      headers: {}, // none required
      query: {
        // add known query params if you have them, e.g.:
        // status: 'optional', region: 'optional', from: 'optional', to: 'optional'
      },
      pagination: {
        type: 'none', // change to 'cursor' | 'page' if your API supports it
      },
      response: {
    ...
    semantics: {
      entityType: 'NaturalDisaster',
      language: 'en',
      keywords: ['NATURALDISASTER', '112', 'alerts', 'incidents'],
      // optionally provide minimal field hints for consumers
      fields: [
        // { name: 'id', type: 'string' },
    ...
    documentation: DATA_DOC_URL || null,
    notes:
      'No auth required. This artifact delivers a formalized endpoint description, not a secret-bearing config.',
  };

  const artResp = await axios.post(`${PROVIDER_URL}/api/artifacts`, {
    title: 'Naturaldisaster Endpoint Config',
    description: 'Structured JSON describing the public Naturaldisaster API endpoint',
  ...
  const reprResp = await axios.post(`${PROVIDER_URL}/api/representations`, {
    title: 'Naturaldisaster JSON Representation',
    mediaType: 'application/json',
  ...
  // 15. Consumer: Discover Catalog of the provider (Naturaldisaster)
  console.log('Discovering catalog');
  const descResp = await axios.post(`${CONSUMER_URL}/api/ids/description`, null, {
    params: { recipient: `${PROVIDER_ALIAS}/api/ids/data` }, // e.g. https://connectornaturaldisaster:8084/api/ids/data
  });
  ...
  console.log('Formalized endpoint config received:', JSON.stringify(config, null, 2));
